...
export const confirmDelete = async (items: readonly ExplorerItem[], isTest: boolean = false): Promise<boolean> => {
  ...
  const result = await ConfirmPrompt.confirm(message, isTest)
  return result
}
